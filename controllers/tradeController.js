...
exports.delete = (req, res, next) => {
  ...
  tradeItem
    .findOneAndDelete({ offerid: id })
    .then((result) => {
      if (result) {
        console.log("Item details deleted");
      } else {
        tradeItem
          .findOneAndDelete({ tradeid: id })
          .then((Item) => {
            if (Item) {
              console.log("Item details deleted");
            }
  ...
  tradeItem
    ...
    .then((result) => {
      ...
      Item.findByIdAndUpdate(
        ...
        .catch((err) => next(err));
      tradeItem
        .findOneAndDelete({ tradeid: tradeId })
        .then((result) => {
          if (result) {
            req.flash("success", "Offer has been accepted. Item traded");
...
exports.tradeoffered = (req, res, next) => {
  ...
  tradeItem
    .findOne({ tradeid: offeredId }, { offerid: tmade1 })
    .then((action) => {
      if (!action) {
...
exports.tradeitem = (req, res, next) => {
  ...
  Promise.all([model.findById(userId), Item.find({ host: userId })])
    .then((results) => {
      ...
      res.render("./trades/tradePhone", { user, trades, offeredId });
    })
    .catch((err) => next(err));
};
...
exports.cancelOffer = (req, res, next) => {
  ...
  tradeItem.findOneAndDelete({ tradeid: phoneId }).then((item) => {
...
exports.index = (req, res, next) => {
  let genres = [];
  Item.distinct("phone_company", function (error, results) {
    genres = results;
  });
  Item.find()
    .then((trades) => res.render("./trades/index", { trades, genres }))
    .catch((err) => next(err));
};
  ...
  Item.findById(id)
    .then((item) => {
      if (item) {
        ...
        let userId = req.session.user;
        watchList
          .findOne({ username: userId, tradeList: id })
          .then((x) => {
            if (x) {
              watch = true;
              item.createAt = DateTime.fromSQL(item.createAt).toFormat(
                "LLLL dd, yyyy"
              );
